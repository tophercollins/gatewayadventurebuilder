...
class SessionDetailData {
  ...
  String get actionItemsSnippet {
    ...
    final openItems =
        actionItems.where((a) => a.status == ActionItemStatus.open).toList();
...
final sessionDetailProvider = FutureProvider.autoDispose
  ...
  final campaignWithWorld =
      await campaignRepo.getCampaignWithWorld(params.campaignId);
  ...
  final appearances =
      await entityRepo.getAppearancesBySession(params.sessionId);
  ...
  final locations = allLocations.where((l) => locationIds.contains(l.id)).toList();
  ...
  final campaignPlayers = await playerRepo.getPlayersByCampaign(params.campaignId);
...
final sessionEntitiesProvider = FutureProvider.autoDispose.family<
    ({List<Npc> npcs, List<Location> locations, List<Item> items})?,
    ({String campaignId, String sessionId})>((ref, params) async {
  final entityRepo = ref.watch(entityRepositoryProvider);
  final campaignRepo = ref.watch(campaignRepositoryProvider);
...
final sessionActionItemsProvider =
    FutureProvider.autoDispose.family<List<ActionItem>, String>(
        (ref, sessionId) async {
  final actionRepo = ref.watch(actionItemRepositoryProvider);
...
final sessionPlayerMomentsProvider = FutureProvider.autoDispose.family<
...
final campaignSessionsProvider =
